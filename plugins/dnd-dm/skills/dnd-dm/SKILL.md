...
4. **Query the adventure book for upcoming content**:
   ...
   cd ~/.claude/skills/candlekeep && uv run candlekeep pages <book-id> -p "<next-section-pages>"
   ```
...
2. **Review the table of contents** to understand structure:
   ```bash
   cd ~/.claude/skills/candlekeep && uv run candlekeep toc <book-id>
   ```
...
1. **Reference the adventure book** when needed:
   ```bash
   cd ~/.claude/skills/candlekeep && uv run candlekeep pages <book-id> -p "<page-range>"
   ```
...
cd ~/.claude/skills/candlekeep && uv run candlekeep toc 9

# Get pages 21-23 (e.g., goblin ambush encounter)
cd ~/.claude/skills/candlekeep && uv run candlekeep pages 9 -p "21-23"

# Get a specific chapter
cd ~/.claude/skills/candlekeep && uv run candlekeep pages 9 -p "14-20"
```
